...
rent_index = (
    system.parameters.gov.economic_assumptions.indices.obr.rent
)
...
def create_modified_cpi_forecast(
    ...
    cpi_values = get_parameter_values(cpi_parameter, forecast_start_year, forecast_end_year)
    water_values = get_parameter_values(water_index_parameter, forecast_start_year, forecast_end_year)
    rent_values = get_parameter_values(rent_index_parameter, forecast_start_year, forecast_end_year)
    ...
    current_weights['w_other'] = 1 - (current_weights['w_water'] + 
    ...
    for year in forecast_years:
        ...
        housing_contribution = (current_weights['w_water'] * pi_water + 
        ...
        total_weighted_growth = (current_weights['w_water'] * (1 + pi_water) + 
                                       current_weights['w_rent'] * (1 + pi_rent) + 
                                       current_weights['w_repair'] * (1 + pi_repair) + 
                                       current_weights['w_other'] * (1 + pi_other))
                
        current_weights['w_water'] = current_weights['w_water'] * (1 + pi_water) / total_weighted_growth
        current_weights['w_rent'] = current_weights['w_rent'] * (1 + pi_rent) / total_weighted_growth
        current_weights['w_repair'] = current_weights['w_repair'] * (1 + pi_repair) / total_weighted_growth
        current_weights['w_other'] = current_weights['w_other'] * (1 + pi_other) / total_weighted_growth
