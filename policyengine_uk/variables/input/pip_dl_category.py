...
class pip_dl_category(Variable):
    ...
    def formula(person, period, parameters):
        p = parameters(period).baseline.gov.dwp.pip.daily_living
        ...
        return select(
            [
                reported_weekly_pip_dl >= p.enhanced * (1 - SAFETY_MARGIN),
                reported_weekly_pip_dl >= p.standard * (1 - SAFETY_MARGIN),
                True,
            ],
            [
