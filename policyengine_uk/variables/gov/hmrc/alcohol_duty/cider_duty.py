...
class cider_duty(Variable):
    value_type = float
    entity = Household
    definition_period = YEAR
    unit = GBP
    label = "Alcohol duty on cider"
    ...
    def formula_2024(household, period, parameters):
        ...
        litres = max_(household("cider_litres", period), 0)
        abv = clip(household("cider_abv", period), 0, 1)
        draught = clip(household("cider_draught_share", period), 0, 1)
        total = 0
        for rates, share in fiscal_year_segments(
            parameters.gov.hmrc.alcohol_duty.rates, period.start.year
        ):
            standard = rates.cider
            draught_rate = rates.draught_cider
            rate = select(
                [
                    abv <= p.taxable_abv,
                    abv < p.standard_abv,
