...
class spirits_duty(Variable):
    value_type = float
    entity = Household
    definition_period = YEAR
    unit = GBP
    label = "Alcohol duty on spirits"
    ...
    def formula_2024(household, period, parameters):
        ...
        litres = max_(household("spirits_litres", period), 0)
        abv = clip(household("spirits_abv", period), 0, 1)
        draught = clip(household("spirits_draught_share", period), 0, 1)
        total = 0
