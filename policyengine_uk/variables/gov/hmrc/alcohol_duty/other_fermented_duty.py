...
class other_fermented_duty(Variable):
    value_type = float
    entity = Household
    definition_period = YEAR
    unit = GBP
    label = "Alcohol duty on other fermented"
    ...
    def formula_2024(household, period, parameters):
        ...
        litres = max_(household("other_fermented_litres", period), 0)
        abv = clip(household("other_fermented_abv", period), 0, 1)
        draught = clip(household("other_fermented_draught_share", period), 0, 1)
        total = 0
        for rates, share in fiscal_year_segments(
            parameters.gov.hmrc.alcohol_duty.rates, period.start.year
        ):
            standard = rates.other
