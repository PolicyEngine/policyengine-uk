...
class sparkling_cider_duty(Variable):
    value_type = float
    entity = Household
    definition_period = YEAR
    unit = GBP
    label = "Alcohol duty on sparkling cider"
    ...
    def formula_2024(household, period, parameters):
        ...
        litres = max_(household("sparkling_cider_litres", period), 0)
        abv = clip(household("sparkling_cider_abv", period), 0, 1)
        draught = clip(household("sparkling_cider_draught_share", period), 0, 1)
        total = 0
        for rates, share in fiscal_year_segments(
            parameters.gov.hmrc.alcohol_duty.rates, period.start.year
        ):
            standard = where(abv <= p.sparkling_cider_abv, rates.cider, rates.other)
            draught_rate = where(
                abv <= p.sparkling_cider_abv, rates.draught_cider, rates.draught_other
            )
            rate = select(
                [
                    abv <= p.taxable_abv,
