...
class wine_duty(Variable):
    value_type = float
    entity = Household
    definition_period = YEAR
    unit = GBP
    label = "Alcohol duty on wine"
    ...
    def formula_2024(household, period, parameters):
        ...
        litres = max_(household("wine_litres", period), 0)
        abv = clip(household("wine_abv", period), 0, 1)
        draught = clip(household("wine_draught_share", period), 0, 1)
        total = 0
        for rates, share in fiscal_year_segments(
            ...
            strength = where(
                rates.wine_easement
                & (abv >= p.wine_easement_min_abv)
                & (abv <= p.wine_easement_max_abv),
                p.wine_easement_abv,
                abv,
            )
