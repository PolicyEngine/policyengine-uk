...
class free_childcare_15_hours_eligibility(Variable):
    ...
    documentation = (
        "Whether the benefit unit is eligible for 15 hours of free childcare"
    )
    ...
    def formula(benunit, period, parameters):
        ...
        age_eligible = benunit.any(
            benunit.members("free_childcare_15_hours", period)
        )

        # Income requirements
        meets_income = benunit.any(
            benunit.members("income_requirements", period)
        )

        # Alternative eligibility
        alternative_eligible = benunit.any(
            benunit.members("work_eligibility_childcare", period)
        )
...
class free_childcare_30_hours_eligibility(Variable):
    ...
    documentation = (
        "Whether the benefit unit is eligible for 30 hours of free childcare"
    )
    definition_period = YEAR
    ...
    def formula(benunit, period, parameters):
        ...
        # Age eligibility
        age_eligible = benunit.any(
            benunit.members("free_childcare_30_hours", period)
        )
