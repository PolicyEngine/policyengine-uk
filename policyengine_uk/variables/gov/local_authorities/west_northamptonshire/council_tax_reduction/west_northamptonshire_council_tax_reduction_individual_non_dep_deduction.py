...
class west_northamptonshire_council_tax_reduction_individual_non_dep_deduction(
    Variable
):
    value_type = float
    entity = Person
    label = (
        "West Northamptonshire Council Tax Reduction individual non-dependant deduction"
    )
    ...
    def formula(person, period, parameters):
        ...
        uc_no_earned_income = has_uc_award & (
            person(
                "west_northamptonshire_council_tax_reduction_non_dep_uc_earned_income",
                period,
            )
            <= 0
        )
