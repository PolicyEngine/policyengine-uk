...
class west_northamptonshire_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "West Northamptonshire Council Tax Reduction"
    ...
    def formula(benunit, period, parameters):
        ctr = parameters(
            period
        ).gov.local_authorities.west_northamptonshire.council_tax_reduction
        local_scheme = benunit(
            "west_northamptonshire_council_tax_reduction_is_local_scheme", period
        )
        full_support = benunit(
            "west_northamptonshire_council_tax_reduction_full_support_group", period
        )
        support_rate = where(
            full_support,
        ...
        ) + benunit("west_northamptonshire_council_tax_reduction_tariff_income", period)
        uc_applicable_income = benunit(
            "west_northamptonshire_council_tax_reduction_uc_applicable_income",
            period,
        )
        ...
        non_dep_deductions = benunit(
            "west_northamptonshire_council_tax_reduction_non_dep_deductions",
            period,
        )
        award = max_(
            0,
