...
class camden_council_tax_reduction_childcare_deduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Camden CTR childcare deduction from earnings"
    ...
    def formula(benunit, period, parameters):
        ...
        unable_to_work = (
            (person("pip_dl", period) > 0) | (person("dla_sc", period) > 0)
        ) & claimant_or_partner
