...
class ipswich_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Ipswich Council Tax Reduction"
    ...
    def formula(benunit, period, parameters):
        ...
        local_scheme = benunit("ipswich_council_tax_reduction_is_local_scheme", period)
        has_uc_award = benunit("universal_credit", period) > 0
        liability = benunit.household("council_tax", period)
        non_dep_deductions = benunit(
            "ipswich_council_tax_reduction_non_dep_deductions", period
        )
        ...
        ) + benunit("ipswich_council_tax_reduction_tariff_income", period)
        ...
        weekly_uc_contribution = benunit(
            "ipswich_council_tax_reduction_uc_weekly_contribution", period
        )
        uc_award = max_(
            0,
