...
class ipswich_council_tax_reduction_individual_non_dep_deduction(Variable):
    value_type = float
    entity = Person
    label = "Ipswich Council Tax Reduction individual non-dependant deduction"
    definition_period = YEAR
    unit = GBP
    defined_for = "council_tax_reduction_individual_non_dep_deduction_eligible"

    def formula(person, period, parameters):
        ctr = parameters(period).gov.local_authorities.ipswich.council_tax_reduction
        ...
        local_scheme = household.any(
            ...
            & household_person.benunit(
                "ipswich_council_tax_reduction_is_local_scheme",
                period,
            )
        )
        ...
        uc_no_earned_income = has_uc_award & (
            person("ipswich_council_tax_reduction_non_dep_uc_earned_income", period)
            <= 0
        )
        exempt = (
            claimant_exempt
            ...
            | income_based_benefit
            | uc_no_earned_income
            | person("ipswich_council_tax_reduction_non_dep_source_exemption", period)
        )
