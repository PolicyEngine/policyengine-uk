...
class rushmoor_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Rushmoor Council Tax Reduction"
    definition_period = YEAR
    unit = GBP
    reference = "https://www.rushmoor.gov.uk/media/qvmlpekv/rushmoor-s13a-scheme-202627-final.pdf"

    def formula(benunit, period, parameters):
        ctr = parameters(period).gov.local_authorities.rushmoor.council_tax_reduction
        local_scheme = benunit("rushmoor_council_tax_reduction_is_local_scheme", period)
        has_uc_award = (
            max_(
        ...
        applicable_income = where(
            has_uc_award,
            benunit("rushmoor_council_tax_reduction_uc_applicable_income", period),
        ...
        liability = benunit.household(
            "rushmoor_council_tax_reduction_maximum_eligible_liability", period
        )
        non_dep_deductions = benunit(
            "rushmoor_council_tax_reduction_non_dep_deductions", period
        )
        award = max_(
            0,
