...
class mid_suffolk_council_tax_reduction_individual_non_dep_deduction(Variable):
    value_type = float
    entity = Person
    label = "Mid Suffolk Council Tax Reduction individual non-dependant deduction"
    definition_period = YEAR
    unit = GBP
    defined_for = "council_tax_reduction_individual_non_dep_deduction_eligible"

    def formula(person, period, parameters):
        ctr = parameters(period).gov.local_authorities.mid_suffolk.council_tax_reduction
        ...
        local_scheme = household.any(
            ...
            & household_person.benunit(
                "mid_suffolk_council_tax_reduction_is_local_scheme",
                period,
            )
        )
        ...
        uc_no_earned_income = has_uc_award & (
            person("mid_suffolk_council_tax_reduction_non_dep_uc_earned_income", period)
            <= 0
        )
        exempt = (
            claimant_exempt
            ...
            | uc_no_earned_income
            | person(
                "mid_suffolk_council_tax_reduction_non_dep_source_exemption", period
            )
        )
