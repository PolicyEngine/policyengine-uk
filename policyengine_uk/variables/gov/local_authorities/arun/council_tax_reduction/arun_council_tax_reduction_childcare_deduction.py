...
class arun_council_tax_reduction_childcare_deduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Arun Council Tax Reduction childcare deduction from earnings"
    ...
    def formula(benunit, period, parameters):
        childcare = parameters(
            period
        ).gov.local_authorities.arun.council_tax_reduction.childcare
        ...
        unable_to_work = (
            ...
            | person(
                "arun_council_tax_reduction_childcare_incapacitated_partner", period
            )
        ) & claimant_or_partner
        couple_one_working_one_unable = (
            benunit("is_couple", period)
            & benunit.any(claimant_or_partner & works_enough_hours)
            & benunit.any(unable_to_work)
        )
        eligible_work = (
            lone_parent_working | couple_both_working | couple_one_working_one_unable
        )
        eligible_child = child_or_young_person & (
            ...
            | (
                ...
                & (
                    ...
                    | person(
                        "arun_council_tax_reduction_childcare_disabled_child", period
                    )
                )
            )
        )
        ...
        return (
            ~has_uc_award
            * eligible_work
            * min_(childcare_expenses, weekly_maximum * WEEKS_IN_YEAR)
        )
