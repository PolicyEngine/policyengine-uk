...
class east_hampshire_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "East Hampshire Council Tax Support"
    ...
    def formula(benunit, period, parameters):
        ...
        applicable_income = where(
            has_uc_award,
            benunit(
                "east_hampshire_council_tax_reduction_uc_applicable_income", period
            ),
        ...
        non_dep_deductions = benunit(
            "east_hampshire_council_tax_reduction_non_dep_deductions", period
        )
        award = max_(
            0,
