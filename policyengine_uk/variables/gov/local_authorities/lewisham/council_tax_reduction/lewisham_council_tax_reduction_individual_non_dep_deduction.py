...
class lewisham_council_tax_reduction_individual_non_dep_deduction(Variable):
    value_type = float
    entity = Person
    label = "Lewisham Council Tax Reduction individual non-dependant deduction"
    ...
    def formula(person, period, parameters):
        ...
        working_age = is_lewisham_working_age(
        ...
        earned_income = where(
            ...
            person.benunit.sum(person_earned_income),
        )
        ...
        uc_no_earned_income = has_uc_award & (
            person.benunit.sum(person_earned_income) <= 0
        )
        exempt = (
            claimant_exempt
