...
class lewisham_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Lewisham Council Tax Reduction"
    ...
    def formula(benunit, period, parameters):
        ...
        working_age = is_lewisham_working_age(local_authority, country, has_pensioner)
        ...
        ) + benunit("lewisham_council_tax_reduction_tariff_income", period)
        uc_applicable_income = benunit(
            "lewisham_council_tax_reduction_uc_applicable_income", period
        )
        applicable_income = where(
            has_uc_case, uc_applicable_income, non_uc_applicable_income
        )
        ...
        non_dep_deductions = benunit(
            "lewisham_council_tax_reduction_non_dep_deductions", period
        )
        award = max_(
            0,
        ...
        capital_eligible = where(
            has_uc_case, uc_capital_eligible, non_uc_capital_eligible
        )
