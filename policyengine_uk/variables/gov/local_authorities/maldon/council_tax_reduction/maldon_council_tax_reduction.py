...
class maldon_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Maldon Council Tax Reduction"
    definition_period = YEAR
    unit = GBP
    reference = "https://democracy.maldon.gov.uk/documents/s40932/Appendix%203.pdf"

    def formula(benunit, period, parameters):
        ctr = parameters(period).gov.local_authorities.maldon.council_tax_reduction
        local_scheme = benunit("maldon_council_tax_reduction_is_local_scheme", period)
        has_uc_award = (
            max_(
        ...
        applicable_income = where(
            has_uc_award,
            benunit("maldon_council_tax_reduction_uc_applicable_income", period),
        ...
        liability = benunit.household(
            "maldon_council_tax_reduction_maximum_eligible_liability", period
        )
        non_dep_deductions = benunit(
            "maldon_council_tax_reduction_non_dep_deductions", period
        )
        award = max_(
            0,
