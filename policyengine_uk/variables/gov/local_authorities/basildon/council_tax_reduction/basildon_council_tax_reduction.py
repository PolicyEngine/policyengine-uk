...
class basildon_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Basildon Council Tax Reduction"
    definition_period = YEAR
    unit = GBP

    def formula(benunit, period, parameters):
        ctr = parameters(period).gov.local_authorities.basildon.council_tax_reduction
        local_scheme = benunit("basildon_council_tax_reduction_is_local_scheme", period)
        support_rate = benunit("basildon_council_tax_reduction_support_rate", period)
