...
class kingston_upon_hull_council_tax_reduction_individual_non_dep_deduction(Variable):
    value_type = float
    entity = Person
    label = (
        "Kingston upon Hull Council Tax Reduction individual non-dependant deduction"
    )
    ...
    def formula(person, period, parameters):
        ctr = parameters(
            period
        ).gov.local_authorities.kingston_upon_hull.council_tax_reduction
        ...
        uc_weekly_hours = (
            person.benunit.sum(person("uc_mif_capped_earned_income", period))
            / WEEKS_IN_YEAR
            / person.benunit.max(person("minimum_wage", period))
        )
        in_remunerative_work = (
            ...
            & (uc_weekly_hours >= ctr.non_dep_deduction.remunerative_work_hours)
        )
        ...
        uc_no_earned_income = (person.benunit("universal_credit", period) > 0) & (
            person.benunit("uc_earned_income", period) <= 0
        )
        exempt = (
            claimant_exempt
