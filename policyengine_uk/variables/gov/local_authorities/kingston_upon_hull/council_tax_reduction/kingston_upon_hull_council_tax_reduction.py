...
class kingston_upon_hull_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Kingston upon Hull Council Tax Reduction"
    ...
    def formula(benunit, period, parameters):
        ...
        uc_applicable_income = benunit(
            "kingston_upon_hull_council_tax_reduction_uc_applicable_income", period
        )
        ...
        non_dep_deductions = benunit(
            "kingston_upon_hull_council_tax_reduction_non_dep_deductions", period
        )
        maximum_award = (
            max_(0, liability - non_dep_deductions) * ctr.maximum_support_rate
        )
        award = max_(
            0,
            maximum_award - excess_income * ctr.means_test.withdrawal_rate,
        )
