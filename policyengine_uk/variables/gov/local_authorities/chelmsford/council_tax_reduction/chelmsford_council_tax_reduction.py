...
class chelmsford_council_tax_reduction(Variable):
    value_type = float
    entity = BenUnit
    label = "Chelmsford Local Council Tax Support"
    ...
    def formula(benunit, period, parameters):
        ...
        non_uc_applicable_income = benunit(
            "chelmsford_council_tax_reduction_applicable_income", period
        )
        uc_applicable_income = benunit(
            "chelmsford_council_tax_reduction_uc_applicable_income", period
        )
        ...
        liability = benunit.household(
            "chelmsford_council_tax_reduction_maximum_eligible_liability", period
        )
        non_dep_deductions = benunit(
            "chelmsford_council_tax_reduction_non_dep_deductions", period
        )
        award = max_(
            0,
