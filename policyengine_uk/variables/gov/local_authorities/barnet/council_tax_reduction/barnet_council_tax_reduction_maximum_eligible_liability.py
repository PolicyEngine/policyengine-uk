...
class barnet_council_tax_reduction_maximum_eligible_liability(Variable):
    value_type = float
    entity = Household
    label = "Barnet maximum Council Tax liability eligible for CTS"
        ...
        protected = (
            household.max(
                household.members.benunit(
                    "barnet_council_tax_reduction_protected_group", period
                )
            )
            > 0
        )
