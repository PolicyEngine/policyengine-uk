...
class babergh_council_tax_reduction_individual_non_dep_deduction(Variable):
    value_type = float
    entity = Person
    label = "Babergh Council Tax Reduction individual non-dependant deduction"
    ...
    def formula(person, period, parameters):
        ...
        local_scheme = household.any(
            ...
            & household_person.benunit(
                "babergh_council_tax_reduction_is_local_scheme",
                period,
            )
        )
        ...
        uc_no_earned_income = has_uc_award & (
            person("babergh_council_tax_reduction_non_dep_uc_earned_income", period)
            <= 0
        )
        exempt = (
            claimant_exempt
            ...
            | income_based_benefit
            | uc_no_earned_income
            | person("babergh_council_tax_reduction_non_dep_source_exemption", period)
        )
