...
class croydon_council_tax_reduction_maximum_eligible_liability(Variable):
    value_type = float
    entity = Household
    label = "Croydon maximum Council Tax liability eligible for Council Tax Support"
    ...
    def formula(household, period, parameters):
        ...
        disabled_not_working = household.any(
            claimant_benunit
            & person.benunit(
                "croydon_council_tax_reduction_disabled_not_working", period
            )
        )
        capped = working_age & ~disabled_not_working & (band_ratio > cap_band_ratio)
