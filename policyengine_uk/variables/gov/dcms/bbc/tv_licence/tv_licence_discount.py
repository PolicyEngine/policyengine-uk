...
class tv_licence_discount(Variable):
    ...
    def formula(household, period, parameters):
        ...
        # Aged discount
        aged = person("age", period) >= p.discount.aged.min_age
        has_aged = household.any(aged)
        claims_pc = add(household, period, ["pension_credit"]) > 0
        meets_pc_requirement = not_(p.discount.aged.must_claim_pc) | claims_pc
        eligible_for_aged_discount = has_aged & meets_pc_requirement
        aged_discount = eligible_for_aged_discount * p.discount.aged.discount

        # Blind discount
        is_blind = person("is_blind", period)
        has_blind = household.any(is_blind)
        blind_discount = has_blind * p.discount.blind.discount
