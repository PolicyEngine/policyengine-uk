...
yaml_file = Path(__file__).parent / "behavioral_responses" / "test_labor_supply_responses.yaml"
...
class TestBehavioralResponses:
    ...
    def test_obr_parameter_functionality(self):
        ...
        # Test enabling OBR
        scenario_on = Scenario(parameter_changes={
            "gov.dynamic.obr_labour_supply_assumptions": {"2025": True}
        })
        sim_on = Microsimulation(scenario=scenario_on)
        obr_on = sim_on.tax_benefit_system.parameters.gov.dynamic.obr_labour_supply_assumptions("2025")
        
        # Test disabling OBR
        scenario_off = Scenario(parameter_changes={
            "gov.dynamic.obr_labour_supply_assumptions": {"2025": False}
        })
        sim_off = Microsimulation(scenario=scenario_off)
        obr_off = sim_off.tax_benefit_system.parameters.gov.dynamic.obr_labour_supply_assumptions("2025")
    ...
    def test_dynamics_no_crash_simple(self):
        ...
        try:
            ...
            if dynamics is not None:
                assert hasattr(dynamics, 'fte_impacts'), "Dynamics should have fte_impacts attribute"
    ...
    def test_basic_behavioral_response_enabled(self):
        ...
        obr_enabled = reformed.tax_benefit_system.parameters.gov.dynamic.obr_labour_supply_assumptions("2025")
    ...
    def test_all_yaml_cases_structure(self, test_case):
        ...
        assert len(situation['people']) > 0, f"Test case '{test_case['name']}' should have people"
        
        # Should be able to calculate basic variables
        employment_income = reformed.calculate("employment_income", test_case['period'])
        assert employment_income is not None, f"Should be able to calculate employment_income for '{test_case['name']}'"
