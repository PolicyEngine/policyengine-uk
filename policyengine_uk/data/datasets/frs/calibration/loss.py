...
CALIBRATION_PARAMETER_FOLDER = (
    STORAGE_FOLDER.parent.parent / "parameters" / "calibration"
)


def generate_model_variables(
    dataset: str, time_period: str = "2023", no_weight_adjustment=False
) -> Tuple:
    ...
    capped_household_size = simulation.calculate(
    ...
    for variable in [
        ...
        for i in range(len(income_parameter.amount.thresholds)):
            ...
            upper_threshold = (
                income_parameter.amount.thresholds[i + 1]
                if i + 1 < len(income_parameter.amount.thresholds)
                else np.inf
            )
    ...
    for program in parameters.obr.program_forecasts.children:
        obr_program = parameters.obr(current_instant).program_forecasts[
            program
        ]
