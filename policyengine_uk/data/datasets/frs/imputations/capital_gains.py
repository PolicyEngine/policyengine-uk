...
capital_gains["maximum_total_income"] = capital_gains.minimum_total_income.shift(-1).fillna(np.inf)
...
def impute_capital_gains(total_income: float, age: float) -> float:
    if total_income < 0 or age < 18:
        return 0
    distribution_row = capital_gains[(capital_gains["minimum_total_income"] <= total_income) & (capital_gains["maximum_total_income"] > total_income)]
    percent_with_gains = distribution_row["percent_with_gains"].values[0]
    has_gains = np.random.choice([0, 1], p=[1-percent_with_gains, percent_with_gains])
    if not has_gains:
        return 0
...
if __name__ == "__main__":
    imputed_gains = []
    for income, age in tqdm(list(zip(total_income, sim.calculate("age", 2023)))):
        imputed_gains.append(impute_capital_gains(income, age))

    pd.DataFrame({"imputed_gains": imputed_gains}).to_csv(STORAGE_FOLDER / "imputations" / "imputed_gains.csv.gz", index=False)
