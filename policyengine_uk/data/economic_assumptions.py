...
def create_policyengine_uprating_factors_table(print_diff=True):
    ...
    for year in range(END_YEAR, START_YEAR, -1):
        ...
        df_growth[year_str] = round(df_growth[year_str] / df_growth[prev_year_str] - 1, 3)
...
def print_csv_diff(old_df, new_df):
    ...
    if changes:
        ...
        print(f"{'Variable':<30} {'Column':<15} {'Old value':<15} {'New value':<15}")
        print("-" * 75)
...
def convert_yoy_growth_to_index(
    ...
    year_columns = [col for col in growth_factors.columns if col not in ['Variable', 'Parameter']]
...
BASELINE_GROWFACTORS = create_policyengine_uprating_factors_table(print_diff=False)
