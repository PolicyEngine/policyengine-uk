...
def drop_zero_weight_households(dataset):
    ...
    is_zero_weight_all_years = np.ones_like(sim.calculate("household_weight").values)
    ...
    person_zero_weight = sim.map_result(is_zero_weight_all_years, "household", "person")
    ...
    for variable in data:
        ...
        for time_period in data[variable]:
            if entity in entity_to_remove:
                data[variable][time_period] = data[variable][time_period][~entity_to_remove[entity]]
