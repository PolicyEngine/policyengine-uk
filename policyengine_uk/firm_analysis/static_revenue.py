...
print("Loading data...")
synthetic_firms = pd.read_csv(
    os.path.join(current_dir, "synthetic_firms_turnover.csv")
)

weighted_total_firms = synthetic_firms["weight"].sum()
print(
    f"Loaded {weighted_total_firms:,.0f} firms with pre-calculated VAT liability"
)

# Check if VAT liability already exists in the data
if "vat_liability_k" not in synthetic_firms.columns:
    print(
        "ERROR: vat_liability_k column not found. Please run generate_synthetic_data.py first."
    )
    exit(1)
...
print(
    f"  Total VAT liability: £{(synthetic_firms['vat_liability'] * synthetic_firms['weight']).sum() / 1_000_000:,.2f} million"
)
print(
    f"  Firms with positive VAT: {synthetic_firms[synthetic_firms['vat_liability'] > 0]['weight'].sum():,.0f}"
)
print(
    f"  Firms with negative VAT: {synthetic_firms[synthetic_firms['vat_liability'] < 0]['weight'].sum():,.0f}"
)
print(
    f"  Firms with zero VAT: {synthetic_firms[synthetic_firms['vat_liability'] == 0]['weight'].sum():,.0f}"
)

# Static revenue impact analysis
# Convert turnover from thousands to pounds for threshold comparison
synthetic_firms["annual_turnover"] = (
    synthetic_firms["annual_turnover_k"] * 1000
)
...
print(
    f"Percentage of firms affected: {affected_weighted_firms/total_weighted_firms*100:.2f}%"
)
print(
    f"\nTotal VAT liability in bunching region: £{total_revenue_loss_millions:,.2f} million"
)
print(
    f"Average VAT per affected firm: £{total_revenue_loss/affected_weighted_firms:,.2f}"
    if affected_weighted_firms > 0
    else "N/A"
)
...
if len(affected) > 0:
    ...
    sector_impact = affected.groupby("sic_code").apply(
        lambda x: pd.Series(
            {
                "weighted_vat_liability": (
                    x["vat_liability"] * x["weight"]
                ).sum(),
    ...
    sector_impact = sector_impact.sort_values(
        "weighted_vat_liability", ascending=False
    ).head(5)
    ...
    for idx, (sector, row) in enumerate(sector_impact.iterrows(), 1):
        ...
        print(
            f"   Firms: {row['weighted_firm_count']:,.0f}, VAT: £{vat_millions:.2f} million"
        )
...
print("=" * 60)
print(
    f"Firms with missing SIC codes: {synthetic_firms[synthetic_firms['sic_code'].isna()]['weight'].sum():,.0f}"
)
print(
    f"Firms with zero/negative turnover: {synthetic_firms[synthetic_firms['annual_turnover_k'] <= 0]['weight'].sum():,.0f}"
)
print(
    f"Raw records with zero/negative weights: {(synthetic_firms['weight'] <= 0).sum()}"
)
print(
    f"Firms with non-zero VAT liability: {synthetic_firms[synthetic_firms['vat_liability'] != 0]['weight'].sum():,.0f}"
)

# Calculate total weighted VAT liability
total_weighted_vat = (
    synthetic_firms["vat_liability"] * synthetic_firms["weight"]
).sum()
total_vat_billions = total_weighted_vat / 1_000_000_000
print(
    f"Total weighted VAT liability across all firms: £{total_vat_billions:,.2f} billion"
)

# Show input/output statistics
print(f"\nInput/Output Statistics:")
print(
    f"Average input/output ratio: {(synthetic_firms['annual_input_k'] / synthetic_firms['annual_turnover_k']).mean():.2%}"
)
print(
    f"Firms with input > output (negative VAT): {synthetic_firms[synthetic_firms['annual_input_k'] > synthetic_firms['annual_turnover_k']]['weight'].sum():,.0f}"
)
...
for fy in fiscal_years:
    ...
    adjusted_firms["annual_turnover"] = (
        adjusted_firms["annual_turnover_k"] * 1000 * fy["firm_growth"]
    )
    adjusted_firms["vat_liability"] = (
        adjusted_firms["vat_liability_k"] * 1000 * fy["firm_growth"]
    )
    ...
    if fy["baseline"] > fy["policy"]:
        ...
        pe_impact = (
            baseline_affected["vat_liability"] * baseline_affected["weight"]
        ).sum() / 1_000_000
    else:
        ...
        pe_impact = (
            -(
                baseline_affected["vat_liability"]
                * baseline_affected["weight"]
            ).sum()
            / 1_000_000
        )
...
print("\n" + "-" * 100)
print(
    f"{'Fiscal Year':<12} {'Lagged RPI':<20} {'Baseline':<12} {'Policy':<12} {'HMRC (£m)':<12} {'PE (£m)':<12} {'Diff (£m)':<10}"
)
print("-" * 100)

for _, row in results_df.iterrows():
    print(
        f"{row['Fiscal Year']:<12} {row['Lagged RPI used']:<20} {row['Baseline']:<12} {row['Policy']:<12} "
...
print(
    f"PE = PolicyEngine estimate based on firm-level VAT liability calculations"
)
...
ax.set_title(
    "Revenue Impact of Increasing the VAT Registration Threshold (2024-29)",
    fontsize=14,
    fontweight="bold",
)
ax.set_xticks(x)
