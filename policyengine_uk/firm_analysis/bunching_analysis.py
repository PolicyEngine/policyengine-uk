...
class CounterfactualBunchingAnalysis:
    ...
    def __init__(
        ...
        window_left=10,
        window_right=10,
        new_policy_window_left=10,
        new_policy_window_right=10,
    ):
        """
    ...
    def load_and_prepare_data(self):
        ...
        df_filtered = df[
            (df["annual_turnover_k"] >= 20) & (df["annual_turnover_k"] <= 140)
        ].copy()
    ...
    def fit_counterfactual(self):
        ...
        exclude_mask = (self.bin_centers >= y_minus) & (
            self.bin_centers <= y_plus
        )
    ...
    def compute_masses_in_window(self):
        ...
        mask_N = (self.bin_centers >= self.T_lower) & (
            self.bin_centers < self.T_star
        )
        mask_R = (self.bin_centers >= self.T_star) & (
            self.bin_centers <= self.T_upper
        )
    ...
    def set_effective_wedge(
        ...
        if tau_e is not None:
            ...
            param is not None for param in [lambda_param, rho, tau, s_c, v]
        ):
            ...
            print(
                f"Step 2: Using default effective wedge τₑ = {self.tau_e:.4f}"
            )
    ...
    def calibrate_substitution_elasticity(self):
        ...
        if self.tau_e is None:
            raise ValueError(
                "Must set effective wedge τₑ before calibrating elasticity"
            )
        ...
        if (
            ...
            raise ValueError(
                "All masses must be positive for elasticity calibration"
            )
        ...
        if ratio_of_ratios > 0 and (1 + self.tau_e) > 0:
            ...
            self.sigma = 0
            print(
                "Warning: Invalid values for elasticity calculation, setting σ = 0"
            )

        print(
            f"Step 3: Calibrated substitution elasticity σ = {self.sigma:.4f}"
        )
    ...
    def compute_aggregate_displaced_share(self):
        ...
        if self.tau_e is None or self.sigma is None:
            raise ValueError(
                "Must complete Steps 2-3 before computing displaced share"
            )
    ...
    def compute_local_bunching_probability(self, T_obs):
        ...
        excess_density = np.maximum(
            f_obs_interp(T_obs) - f_cf_interp(T_obs), 0
        )
        ...
        pi_T_obs = (
            excess_density / f_obs_interp(T_obs)
            if f_obs_interp(T_obs) > 0
            else 0
        )
    ...
    def compute_rank_among_bunchers(self, T_obs):
        ...
        start_idx = np.searchsorted(
            self.bin_centers, self.T_lower, side="left"
        )
    ...
    def compute_counterfactual_mapping(self, T_obs):
        ...
        for i, T_obs_val in enumerate(T_obs):
            if self.T_lower <= T_obs_val <= self.T_star:
                ...
                cdf_arg = F_cf_at_threshold + u_val * self.Delta_R / np.sum(
                    self.f_cf
                )
    ...
    def run_step4_analysis(self, sample_turnovers=None):
        ...
        if sample_turnovers is None:
            ...
            sample_turnovers = np.linspace(
                self.T_lower, self.T_star, 11
            )  # 11 points from T*-W to T*

        print(f"Analyzing {len(sample_turnovers)} sample turnover points:")
        print(
            f"Sample range: £{sample_turnovers[0]:.1f}k to £{sample_turnovers[-1]:.1f}k"
        )
        ...
        print(
            f"Average displacement: £{results_df['displacement'].mean():.2f}k"
        )
    ...
    def run_analysis(self, tau_e=None, include_step4=False, **wedge_params):
        ...
        print(
            f"{step_range}: Counterfactual, Wedge, Elasticity"
        ...
        print(
            f"Fitted counterfactual excluding window [{self.T_lower}, {self.T_upper}]"
        )
        ...
        if include_step4:
            ...
            if hasattr(self, "firm_mappings"):
                ...
                self.step6_revenue_mapping(
                    tau_old=0.20, tau_new=0.20
                )  # Same rate, different threshold
    ...
    def step5_forward_map_to_new_policy(self, T_star_new=100, tau_e_new=None):
        ...
        f_new_policy = self.create_new_policy_distribution(
            T_star_new, tau_e_new, Pi_new
        )
        ...
        print(
            f"Step 5 complete: Generated new policy distribution with bunching at £{T_star_new}k"
        )
    ...
    def create_new_policy_distribution(self, T_star_new, tau_e_new, Pi_new):
        ...
        new_region_mask = (self.bin_centers >= T_lower_new) & (
            self.bin_centers <= T_upper_new
        )
        above_threshold_mask = (self.bin_centers > T_star_new) & (
            self.bin_centers <= T_upper_new
        )

        if not np.any(above_threshold_mask):
            print(
                f"Warning: No bins above new threshold £{T_star_new}k in analysis range"
            )
            return f_new
        ...
        if mass_to_move > 0:
            ...
            below_threshold_mask = (self.bin_centers >= T_lower_new) & (
                self.bin_centers <= T_star_new
            )
            ...
            if np.any(below_threshold_mask):
                ...
                print(
                    f"  Redistributed mass to {np.sum(below_threshold_mask)} bins below new threshold"
                )
    ...
    def apply_smoothing(self, distribution):
        ...
        for i in range(5, len(smooth_dist) - 5):
            ...
            smooth_dist[i] = sum(
                w * smooth_dist[i + j - 5] for j, w in enumerate(weights)
            )
    ...
    def step6_revenue_mapping(self, tau_old=0.20, tau_new=0.20):
        ...
        theta_default = (
            0.85  # Share of taxable outputs (85% for typical firms)
        )
        ...
        if not hasattr(self, "firm_mappings"):
            print(
                "Error: Need Step 4 firm mappings. Run step4_advanced_mapping first."
            )
            return
        ...
        print(
            f"Revenue change: £{total_change:,.0f}k ({100*total_change/total_revenue_old:+.2f}%)"
        )
        ...
        print(
            f"  Firms with revenue increase: {len(positive_changes):,} ({100*len(positive_changes)/len(self.revenue_changes):.1f}%)"
        )
        print(
            f"  Firms with revenue decrease: {len(negative_changes):,} ({100*len(negative_changes)/len(self.revenue_changes):.1f}%)"
        )
    ...
    def step7_elasticity_calculations(
        self, tau_old=0.20, epsilon_tau=0.01, epsilon_threshold=5
    ):
        """
        ...
        print(
            f"   d ln(q_R/q_N) / d ln(1+τ_e) = -σ = {behavioral_elasticity:.3f}"
        )
        ...
        print(
            f"   Revenue elasticity w.r.t rate = {revenue_elasticity_rate:.3f}"
        )

        # Revenue elasticity w.r.t threshold (finite differences)
        print(f"\n3) Revenue elasticity w.r.t threshold:")
        print(
            f"   Computing finite differences with ε = £{epsilon_threshold}k..."
        )

        # Simulate T* + ε
        threshold_plus = self.T_star + epsilon_threshold
        revenue_threshold_plus = self.calculate_revenue_for_threshold(
            threshold_plus
        )

        # Simulate T* - ε
        threshold_minus = self.T_star - epsilon_threshold
        revenue_threshold_minus = self.calculate_revenue_for_threshold(
            threshold_minus
        )

        # Revenue elasticity = (dV/V) / (dT*/T*)
        dV_dthreshold = (revenue_threshold_plus - revenue_threshold_minus) / (
            2 * epsilon_threshold
        )
        revenue_elasticity_threshold = (
            dV_dthreshold * self.T_star
        ) / baseline_revenue

        print(f"   dV/dT* = £{dV_dthreshold:,.0f}k per £1k threshold change")
        print(
            f"   Revenue elasticity w.r.t threshold = {revenue_elasticity_threshold:.3f}"
        )
    ...
    def create_sample_firm_mappings(self):
            ...
            if 65 <= T_old <= 110:  # Near thresholds - apply bunching effects
                if hasattr(self, "f_new_policy") and hasattr(self, "f_cf"):
                    ...
                    if T_old < 90:
                        T_new = (
                            T_old * 1.02
                        )  # Small increase due to threshold change
                    else:
        ...
        print(
            f"Created {len(self.firm_mappings):,} sample firm mappings for Steps 6-7"
        )
    ...
    def print_results(self):
        ...
        if has_step4:
            ...
            print(
                f"  Avg bunching probability: {self.step4_results['pi'].mean():.4f}"
            )
            print(
                f"  Avg displacement: £{self.step4_results['displacement'].mean():.2f}k"
            )
    ...
    def plot_results(self, include_step5=False):
        ...
        ax.plot(
            y_dist_plot,
            self.f_obs[plot_mask],
            "o-",
            color="blue",
            markersize=3,
            linewidth=1,
            label="Original Observed f^obs(T) [£90k]",
            alpha=0.7,
        )
        ax.plot(
            y_dist_plot,
        ...
        ax.plot(
            ...
            color="green",
            linewidth=2,
            label="Step 4 Advanced Mapping Result",
            alpha=0.8,
        )
        ...
        if include_step5 and hasattr(self, "f_new_policy"):
            ax.plot(
                y_dist_plot,
                self.f_new_policy[plot_mask],
                "-",
                color="purple",
                linewidth=2,
                label=f"Step 5 New Policy [£{self.T_star_new}k]",
                alpha=0.8,
            )

        # Shade the window (also skip first point)
        window_mask = (self.bin_centers >= self.T_lower) & (
            self.bin_centers <= self.T_upper
        )
        window_mask_plot = window_mask[
            plot_mask
        ]  # Apply same exclusion to window mask
        ax.fill_between(
            y_dist_plot[window_mask_plot],
            0,
            self.f_obs[plot_mask][window_mask_plot],
            alpha=0.3,
            color="lightblue",
            label=f"Window -£{self.W_left}k/+£{self.W_right}k",
        )
        ...
        ax.axvline(
            ...
            linestyle="--",
            alpha=0.5,
            label=f"£90k -£{self.W_left}k/+£{self.W_right}k",
        )
        ...
        if include_step5 and hasattr(self, "T_star_new"):
            new_threshold_dist = (
                self.T_star_new - self.T_star
            )  # Distance from original
            ax.axvline(
                x=new_threshold_dist,
                color="purple",
                linestyle="-",
                linewidth=2,
                label=f"New Threshold £{self.T_star_new}k",
                alpha=0.8,
            )
        ...
        ax.text(
            0.02,
            0.89,
            f"Effective wedge τₑ = {self.tau_e:.3f}",
            transform=ax.transAxes,
            fontsize=11,
        ...
        ax.text(
            0.02,
            0.83,
            f"Substitution elasticity σ = {self.sigma:.3f}",
            transform=ax.transAxes,
            fontsize=11,
        ...
        pi_mean = np.mean(
            [
                self.compute_local_bunching_probability(T)
        ...
        ax.text(
            0.02,
            0.71,
            f"Avg bunching prob π̄ = {pi_mean:.3f}",
            transform=ax.transAxes,
            fontsize=11,
        ...
        if include_step5 and hasattr(self, "Pi_new"):
            ax.text(
                0.02,
                0.65,
                f"New displaced share Π' = {self.Pi_new:.3f}",
                transform=ax.transAxes,
                fontsize=11,
        ...
        # Formatting
        ax.set_xlabel(
            "Distance from Original Threshold £90k (£1,000)", fontsize=12
        )
        ax.set_ylabel("Density f(T)", fontsize=12)

        if include_step5:
            ax.set_title(
                "Steps 1-5: Complete Pipeline with New Policy",
                fontsize=14,
                fontweight="bold",
            )
        else:
            ax.set_title(
                "Steps 1-4: Complete Analysis with Advanced Mapping",
                fontsize=14,
                fontweight="bold",
            )
    ...
    def apply_step4_to_synthetic_firms(
        self, output_file="synthetic_firms_step4_mapped.csv"
    ):
        """
        ...
        try:
            ...
            print(
                f"Loaded {len(df):,} synthetic firms from synthetic_firms_turnover.csv"
            )
        ...
        print(
            f"Firms displaced: {n_displaced:,} ({100*n_displaced/len(df):.2f}%)"
        )
        print(f"Mean displacement: £{displacement.mean():.3f}k")
        if len(nonzero_displacement) > 0:
            print(
                f"Mean nonzero displacement: £{nonzero_displacement.mean():.3f}k"
            )
...
def create_advanced_probabilistic_mapping(
    analysis, decay_rate=0.01, spread_factor=3.0
):
    """
    ...
    print(
        f"- Spread factor: {spread_factor} (controls overall redistribution)"
    )
    ...
    deficit = (
        analysis.f_cf - analysis.f_obs
    )  # Positive = need more, negative = have excess
    ...
    for i, T_obs in enumerate(analysis.bin_centers):
        ...
        if original_density > target_density:
            ...
            if excess_firms > 0:
                ...
                deficit_mask = (deficit > 0) & (
                    np.arange(len(deficit)) > 0
                )  # Exclude index 0
                ...
                if len(target_locations) > 0:
                    ...
                    for j, target_turnover in enumerate(target_locations):
                        target_idx = np.where(
                            analysis.bin_centers == target_turnover
                        )[0][0]
...
def optimize_advanced_parameters(analysis):
    ...
    def mapping_error(params):
        ...
        mapped_dist = create_advanced_probabilistic_mapping(
            analysis, decay_rate, spread_factor
        )
