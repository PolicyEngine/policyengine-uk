...
def create_disable_simulated_benefits() -> Reform:
    """
    Reform that disables simulated benefits and uses reported values instead.

    Policy: Replace calculated benefit amounts with reported values from
    survey data. Useful for validation or when simulating partial reforms.
    """
        ...
        def apply(self):
            ...
            for variable in BENEFITS:
                ...
                if variable in ["child_tax_credit", "working_tax_credit"]:
                    # CTC and WTC have their own pre_minimum variables
                    # because tax credits aren't paid if below a threshold.
...
def disable_simulated_benefits(parameters, period, bypass: bool = False):
    if bypass:
        return create_disable_simulated_benefits()
...
# For direct import
disable_simulated_benefits_reform = create_disable_simulated_benefits()
