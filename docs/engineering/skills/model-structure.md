...
Add `variable` and `parameter_prefix` when the program maps to those model
objects. Add `verified_end_year` when verified coverage has a defined end, and
use `notes` for a concise qualification that does not fit the structured fields.

Change `verified_start_year` or `verified_end_year` only after verifying that
parameters and tests cover the stated years. Current entries use the status
values `complete` and `partial`.
