...
def get_plan_counts(year: int, datasets: dict) -> dict:
    ...
    return {
        plan: person[person["student_loan_plan"] == plan].weights.sum()
        for plan in PLANS
    }
...
def main():
    ...
    for year in YEARS:
        ...
        row = f"{year:>6}  " + "  ".join(
            f"{counts[p]/1e6:>9.3f}m" for p in PLANS
        )
    ...
    for year in YEARS:
        ...
        assert (
            p5 >= prev
        ), f"Plan 5 should not shrink: {year-1}={prev:,.0f} → {year}={p5:,.0f}"
        prev = p5
    ...
    for year in YEARS:
        ...
        assert (
            p1 <= prev
        ), f"Plan 1 should not grow: {year-1}={prev:,.0f} → {year}={p1:,.0f}"
        prev = p1
