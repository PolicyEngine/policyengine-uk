...
def cost_of_reform(reform, period="2020-10-25"):
    ...
    net_cost = np.sum((reformed.calculate("benunit_net_income", period) - baseline.calculate("benunit_net_income", period)) * benunits)
    return net_cost
...
def poverty_reduction(reform, period="2020-10-25"):
    ...
    poverty_baseline = np.average(baseline.calculate("in_poverty_bhc", period), weights=households)
    poverty_reformed = np.average(reformed.calculate("in_poverty_bhc", period), weights=households)
    poverty_reduction = (poverty_baseline - poverty_reformed) / poverty_baseline
    ...
    def loss(x):
        ...
        class income_tax(Variable):
            ...
            def formula(person, period, parameters):
                return flat_tax_rate * person("income_tax_applicable_amount", period)
        ...
        class basic_income(Variable):
            ...
            def formula(person, period, parameters):
                ...
                return senior_amount * seniors + adult_amount * WA_adults + child_amount * children
        ...
        return cost - poverty_impact * 1e+11
    
    solution = differential_evolution(loss, [(0, 1), (0, 200), (0, 200), (0, 200)])
    return solution