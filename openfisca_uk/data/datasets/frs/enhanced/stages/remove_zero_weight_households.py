...
def remove_zero_weight_households(dataset: Dataset, year: int):
    ...
    YEARS = list(range(year, 2027))  # To be removed, households must have zero weight in all of these years

    variables = dataset.keys(year)

    for variable in variables:
        entity = sim.simulation.tax_benefit_system.variables[variable].entity.key
        has_nonzero_weight = sum([sim.calc(f"{entity}_weight", period=year).values for year in YEARS]) > 0
        if dataset.data_format == Dataset.ARRAYS:
            dataset.save(year, variable, dataset.load(year, variable)[has_nonzero_weight])
        elif dataset.data_format == Dataset.TIME_PERIOD_ARRAYS:
            for period in dataset.load(year, variable):
                key = f"{variable}/{period}"
                dataset.save(year, key, dataset.load(year, key)[has_nonzero_weight])
