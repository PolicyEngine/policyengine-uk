...
class Populations(LossCategory):
    ...
    def initialise(self):
        ...
        age_sex_region = self.calibration_parameters.demographics.populations.by_age_sex_region
            ...
            for target_sex in ("MALE", "FEMALE"):
                for target_region in self.sim.calc("region").unique():
                    ...
                    self.comparisons += [(
                        ...
                        self.sim.map_to(meets_all_criteria, "person", "household"),
                        age_sex_region._children[target_sex]._children[target_region]._children[age_string]
                    )]
