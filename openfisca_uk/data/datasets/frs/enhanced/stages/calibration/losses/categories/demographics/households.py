...
class Households(LossCategory):
    ...
    def initialise(self):
        region_ct_band_parameter = self.calibration_parameters.demographics.households.by_region_by_council_tax_band
        ...
        for target_region in region_ct_band_parameter._children:
            for target_ct_band in region_ct_band_parameter._children[target_region]._children:
                self.comparisons.append((
                    f"{target_region}_{target_ct_band}",
                    (ct_band == target_ct_band) & (region == target_region),
                    region_ct_band_parameter._children[target_region]._children[target_ct_band],
                ))
        ...
        for target_region in region_tenure_parameter._children:
            for target_tenure_type in region_tenure_parameter._children[target_region]._children:
                self.comparisons.append((
                    f"{target_region}_{target_tenure_type}",
                    (tenure_type == target_tenure_type) & (region == target_region),
                    region_tenure_parameter._children[target_region]._children[target_tenure_type],
                ))
