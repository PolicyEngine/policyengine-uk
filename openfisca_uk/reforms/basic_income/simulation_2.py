...
def modify_parameters(parameters):
    file_path = os.path.join(dir_name, "parameters", "simulation_2", "new_income_tax.yaml")
...
class income_tax(Variable):
    ...
    def formula(person, period, parameters):
        ...
        weekly_tax = (parameters(period).taxes.income_tax.new_income_tax.calc(estimated_yearly_income)) / 52
        return weekly_tax
...
class basic_income(Variable):
    ...
    def formula(person, period, parameters):
        ...
        return person('is_senior', period) * 45 + adult_young * 50 + adult_old * 60 + person('is_child', period) * 20
