...
class pension_credit_MG(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        personal_allowance = PC.minimum_guarantee[benunit("relation_type", period)] * WEEKS_IN_YEAR
...
class savings_credit_applicable_income(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        exempted_personal_benefits = aggr(benunit, period, [
            "incapacity_benefit",
            "JSA_contrib",
            "ESA_contrib",
            "SDA",
        ...
        exempted_family_benefits = add(benunit, period, [
            "working_tax_credit",
        ])
        return amount_over(GC_income - exempted_personal_benefits - exempted_family_benefits, 0)
...
class pension_credit_SC(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        threshold = SC.income_threshold[benunit("relation_type", period)] * WEEKS_IN_YEAR
        maximum_amount = (1 - SC.withdrawal_rate) * max_(appropriate_amount - threshold, 0)

        amount_A = min_((1 - SC.withdrawal_rate) * max_(income - threshold, 0), maximum_amount)
        amount_B = amount_over(income - appropriate_amount, 0) * SC.withdrawal_rate
        amount = max_(amount_A - amount_B, 0)

        return amount * benunit("pension_credit_eligible", period) * benunit("claims_PC", period)
