...
class is_CTC_child_limit_exempt(Variable):
    ...
    def formula(person, period, parameters):
        limit_year = parameters(period).benefit.tax_credits.child_tax_credit.two_child_limit_year
...
class claims_CTC(Variable):
    ...
    def formula(benunit, period, parameters):
        return aggr(benunit, period, ["child_tax_credit_reported"], options=[ADD]) + (random(benunit) < parameters(period).benefit.tax_credits.child_tax_credit.takeup)
...
class CTC_family_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        return benunit("is_CTC_eligible", period) * benunit("claims_CTC", period) * CTC.elements.family_element
...
class CTC_child_element(Variable):
    ...
    def formula(benunit, period, parameters):
        exempt_children = benunit.sum(benunit.members("is_child_for_CTC", period) * benunit.members("is_CTC_child_limit_exempt", period))
        spaces_left = max_(0, 2 - exempt_children)
        non_exempt_children = min_(spaces_left, benunit.sum(benunit.members("is_child_for_CTC", period)))
...
class CTC_disabled_child_element(Variable):
    ...
    def formula(benunit, period, parameters):
        disabled_children = benunit.sum(benunit.members("is_child_for_CTC", period) * (benunit.members("is_disabled_for_benefits", period, options=[ADD])) > 0)
        CTC = parameters(period).benefit.tax_credits.child_tax_credit
        amount = CTC.elements.dis_child_element * disabled_children
        return benunit("is_CTC_eligible", period) * benunit("claims_CTC", period) * amount
...
class CTC_severely_disabled_child_element(Variable):
    ...
    def formula(benunit, period, parameters):
        severely_disabled_children = benunit.sum(benunit.members("is_child_for_CTC", period) * (benunit.members("is_severely_disabled_for_benefits", period, options=[ADD]) > 0))
        CTC = parameters(period).benefit.tax_credits.child_tax_credit
        amount = CTC.elements.severe_dis_child_element * severely_disabled_children
        return benunit("is_CTC_eligible", period) * benunit("claims_CTC", period) * amount
...
class is_WTC_eligible(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        lower_req = (
            disabled_adults + has_old + (family_type == families.LONE_PARENT) > 0
        )
        medium_req = (family_type == families.COUPLE_WITH_CHILDREN) * (1 - lower_req) > 0
...
class claims_WTC(Variable):
    ...
    def formula(benunit, period, parameters):
        return aggr(benunit, period, ["working_tax_credit_reported"], options=[ADD]) + (random(benunit) < parameters(period).benefit.tax_credits.working_tax_credit.takeup)
...
class WTC_basic_element(Variable):
    ...
    def formula(benunit, period, parameters):
        WTC = parameters(period).benefit.tax_credits.working_tax_credit
        return benunit("is_WTC_eligible", period) * benunit("claims_WTC", period) * WTC.elements.basic
...
class WTC_couple_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        return benunit("is_WTC_eligible", period) * benunit("claims_WTC", period) * amount
...
class WTC_lone_parent_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        family_type = benunit("family_type", period)
        families = family_type.possible_values
        amount = (family_type == families.LONE_PARENT) * WTC.elements.lone_parent
        return benunit("is_WTC_eligible", period) * benunit("claims_WTC", period) * amount
...
class WTC_disabled_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        condition = (
            (
                benunit.sum(
                    (benunit.members("is_disabled_for_benefits", period, options=[ADD]) > 0)
...
class WTC_severely_disabled_element(Variable):
    ...
    def formula(benunit, period, parameters):
        WTC = parameters(period).benefit.tax_credits.working_tax_credit
        amount = benunit("num_severely_disabled_adults", period) * WTC.elements.severely_disabled
        return benunit("is_WTC_eligible", period) * benunit("claims_WTC", period) * amount
...
class WTC_childcare_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        return benunit("is_WTC_eligible", period) * benunit("claims_WTC", period) * childcare_element
...
class tax_credits_reduction(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        threshold = select(
            ...
            [means_test.income_threshold_CTC_only, means_test.income_threshold],
        )
        income = benunit("tax_credits_applicable_income", period)
        reduction = (
            max_(0, (income - threshold))
