...
class land_value(Variable):
    ...
    def formula(household, period, parameters):
        ...
        land_value = parameters(period).wealth.land_value
        property_wealth_intensity = (
            land_value.aggregate_household_land_value / total_property_wealth
        )
        property_wealth_intensity = where(
            total_property_wealth > 0, property_wealth_intensity, 0
        )
        corporate_wealth_intensity = (
            land_value.aggregate_corporate_land_value / total_corporate_wealth
        )
