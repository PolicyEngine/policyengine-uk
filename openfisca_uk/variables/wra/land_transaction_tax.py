...
class ltt_on_residential_property(Variable):
    ...
    def formula(household, period, parameters):
        ...
        rent_tax = ltt.rent.calc(cumulative_rent + rent) - ltt.rent.calc(
            cumulative_rent
        )
...
class ltt_on_non_residential_property(Variable):
    ...
    documentation = (
        "Land Transaction Tax charge on non-residential property value"
    )
    entity = Household
    definition_period = YEAR
    value_type = float
...
class land_transaction_tax(Variable):
    ...
    def formula(household, period):
        return household("ltt_on_residential_property", period) + household(
            "ltt_on_non_residential_property", period
        )
