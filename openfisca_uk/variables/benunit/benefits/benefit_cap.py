...
class benefit_cap(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        rate = select([children * in_london, children * not_(in_london), not_(children) * in_london, not_(children) * not_(in_london)], [cap.london_children, cap.has_children, cap.london_no_children, cap.no_children])
...
class is_benefit_cap_exempt(Variable):
    ...
    def formula(benunit, period, parameters):
        QUAL_PERSONAL_BENEFITS = ["carers_allowance", "DLA_SC", "DLA_M", "ESA_contrib"]
        QUAL_BENUNIT_BENEFITS = ["working_tax_credit", "ESA_income"]
        qualifying = aggr(benunit, period, QUAL_PERSONAL_BENEFITS, options=[MATCH]) + add(benunit, period, QUAL_BENUNIT_BENEFITS, options=[MATCH]) > 0
