...
class UC_premiums(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        childcare_limit = (num_eligible_children == 1) * UC.elements.max_childcare_1 + (
        ...
        premiums = (
            ...
            + benunit("disabled_children", period.this_year) * UC.elements.disabled_child_element
            + benunit("disabled_adults", period.this_year) * UC.elements.disabled_element
        )
...
class UC_eligible_rent(Variable):
    ...
    def formula(benunit, period, parameters):
        UC = parameters(period).benefits.universal_credit
        rent = benunit.max(benunit.members("personal_rent", period, options=[ADD]))
...
class universal_credit_income_reduction(Variable):
    ...
    def formula(benunit, period, parameters):
        UC = parameters(period).benefits.universal_credit
        earned_income = (
            benunit("benunit_earnings", period, options=[DIVIDE])
            - benunit("benunit_tax", period, options=[DIVIDE])
        )
        unearned_income = (
            aggr(benunit, period, ["carers_allowance", "JSA_contrib", "pension_income", "state_pension"], options=[MATCH])
        )
        housing_element = benunit("UC_eligible_rent", period)
        earnings_disregard = (housing_element > 0) * UC.means_test.earn_disregard_with_housing + (housing_element == 0) * UC.means_test.earn_disregard
...
class universal_credit_eligible(Variable):
    ...
    def formula(benunit, period, parameters):
        DISQUALIFYING_BENEFITS = ["working_tax_credit", "child_tax_credit", "housing_benefit", "income_support", "ESA_income", "JSA_income"]
        disqualifying = add(benunit, period, DISQUALIFYING_BENEFITS, options=[MATCH]) + aggr(benunit, period, ["SDA"], options=[MATCH])
        eligible = (disqualifying == 0) * (not_(benunit.min(benunit.members("is_SP_age", period.this_year))))
        return eligible * benunit("universal_credit_reported", period.this_year)
...
class universal_credit(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        capped_amount = min_(final_amount, benunit("benefit_cap", period, options=[ADD]))
