...
class personal_allowance_deduction(Variable):
    ...
    def formula(person, period, parameters):
        return min_(person("personal_allowance", period), person("taxable_income", period))
...
class unused_personal_allowance(Variable):
    ...
    def formula(person, period, parameters):
        return max_(0, person("personal_allowance", period) - person("taxable_income", period))
...
class marriage_allowance(Variable):
    ...
    def formula(person, period, parameters):
        max_amount = parameters(period).taxes.income_tax.allowances.marriage_allowance
        spousal_personal_allowance = person.benunit.sum(person.benunit.members("unused_personal_allowance", period)) - person("unused_personal_allowance", period)
        return min_(max_amount, spousal_personal_allowance * person.benunit("is_married", period))
...
class marriage_allowance_deduction(Variable):
    ...
    def formula(person, period, parameters):
        return min_(person("marriage_allowance", period), person("taxable_income", period))
...
class personal_savings_allowance(Variable):
    ...
    def formula(person, period, parameters):
        ...
        in_basic_threshold = (applicable_amount >= rates.thresholds[0]) * (applicable_amount < rates.thresholds[1])
        in_higher_threshold = (applicable_amount >= rates.thresholds[1]) * (applicable_amount < rates.thresholds[2])
        allowance = where(in_basic_threshold, PSA.basic, where(in_higher_threshold, PSA.higher, PSA.additional))
        return allowance
...
class personal_savings_allowance_deduction(Variable):
    ...
    def formula(person, period, parameters):
        return min_(person("personal_savings_allowance", period), person("savings_interest", period))
...
class savings_starter_allowance_deduction(Variable):
    ...
    def formula(person, period, parameters):
        return min_(person("savings_starter_allowance", period), person("savings_interest", period))
...
class savings_starter_allowance(Variable):
    ...
    def formula(person, period, parameters):
        starter_rate = parameters(period).taxes.income_tax.allowances.savings_starter_rate
        applicable_amount = person("taxable_income", period)
        PA = person("personal_allowance", period)
        starter_allowance = starter_rate.allowance - starter_rate.reduction_rate * max_(0, applicable_amount - PA)
...
class rental_deduction(Variable):
    ...
    def formula(person, period, parameters):
        rental_allowance = parameters(period).taxes.income_tax.allowances.rental_allowance
...
class dividend_deduction(Variable):
    ...
    def formula(person, period, parameters):
        dividend_allowance = parameters(period).taxes.income_tax.allowances.dividend_allowance
...
class dividend_income_tax(Variable):
    ...
    def formula(person, period, parameters):
        ...
        taxable_dividends = person("dividend_income", period) * 52 - person("dividend_deduction", period)
        other_income = person("taxable_income", period)
        basic_amount = max_(0, min_(rates.uk.thresholds[1], other_income + taxable_dividends) - other_income)
        higher_amount = max_(0, min_(rates.uk.thresholds[2], other_income + taxable_dividends) - other_income - rates.uk.thresholds[1])
        additional_amount = higher_amount = max_(0, other_income + taxable_dividends - other_income - rates.uk.thresholds[2])
        dividend_tax = rates.dividend_rates.rates[0] * basic_amount + rates.dividend_rates.rates[1] * higher_amount + rates.dividend_rates.rates[2] * additional_amount
        return dividend_tax
...
class is_higher_earner(Variable):
    ...
    def formula(person, period, parameters):
        higher_earner = person("earned_income", period) > person.benunit.sum(person.benunit.members("earned_income", period)) / 2
        equal_earners = person("earned_income", period) == person.benunit.sum(person.benunit.members("earned_income", period)) / 2
        return higher_earner + equal_earners * person("is_benunit_head", period)
...
class CB_HITC(Variable):
    ...
    def formula(person, period, parameters):
        ...
        phase = min_(HITC.phase_out_end, max_(0, income - HITC.phase_out_start)) / (HITC.phase_out_end - HITC.phase_out_start)
        return phase * person.benunit("child_benefit", period) * person("is_higher_earner", period)
...
class income_tax(Variable):
    ...
    def formula(person, period, parameters):
        income_tax = parameters(period).taxes.income_tax
        applicable_amount = person("taxable_income", period) - person("taxable_income_deductions", period)
        region = person.household("region", period)
        is_in_scotland = region == region.possible_values.SCOTLAND
