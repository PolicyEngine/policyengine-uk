...
class contributory_JSA(Variable):
    ...
    def formula(family, period, parameters):
        ...
        return max_(
            ...
            * family("family_JSA_receipt", period),
        )
...
class income_JSA(Variable):
    ...
    def formula(family, period, parameters):
        ...
        return family("looking_for_work", period) * max_(
...
class income_support(Variable):
    ...
    def formula(family, period, parameters):
        ...
        return max_(
            ...
            * family("family_IS_receipt", period),
        )
...
class working_tax_credit(Variable):
    ...
    def formula(family, period, parameters):
        return (
            max_(
                0,
                family("working_tax_credit_pre_means_test", period)
                - family("child_working_tax_credit_reduction", period),
            )
            / 52
        )
...
class child_tax_credit(Variable):
    ...
    def formula(family, period, parameters):
        ...
        return (
            max_(0, family("child_tax_credit_pre_means_test", period) + reduction_left)
            / 52
        )
...
class child_working_tax_credit_combined(Variable):
    ...
    def formula(family, period, parameters):
        ...
        means_tested_amount = max_(
            0, (child_tax_credit_amount + working_tax_credit_amount) - reduction
        )
...
class benefit_cap_reduction(Variable):
    ...
    def formula(family, period, parameters):
        ...
        benefit_cap = (
            family("is_single", period)
            * parameters(period).benefits.benefit_cap.amount_single
            + (family.nb_persons() > 1)
            * parameters(period).benefits.benefit_cap.amount_family
        )
        affected_benefits = (
