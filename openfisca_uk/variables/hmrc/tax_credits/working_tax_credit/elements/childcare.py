...
class wtc_childcare_element(Variable):
    ...
    def formula(benunit, period, parameters):
        ...
        works_lower_hours = (
            ...
            >= wtc.eligibility.work_requirements.lower
        )
        meets_additional_work_condition = ~(
            benunit.sum(is_adult & ~works_lower_hours) > 0
        )
        num_children = benunit("num_children", period)
        maximum = wtc.elements.childcare.maximum
        max_weekly_childcare = where(
            num_children == 1, maximum.single, maximum.multiple
        )
        ...
        eligible_expenses = (
            min_(max_childcare_amount, expenses)
            * meets_additional_work_condition
        )
