...
class is_ctc_child_limit_exempt(Variable):
    ...
    documentation = "Whether this benefit unit is exempt from the CTC child limit"
    entity = BenUnit
...
class meets_ctc_child_limit(Variable):
    ...
    def formula(person, period, parameters):
        limit_at_date = lambda date: parameters(date).hmrc.tax_credits.child_tax_credit.eligibility.child_limit
        limit_when_born = np.array([limit_at_date(instant(birth_date)) for birth_date in person("birth_date", period).astype(date)])
