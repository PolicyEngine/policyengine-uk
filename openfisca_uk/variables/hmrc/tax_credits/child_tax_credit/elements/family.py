...
class ctc_family_element(Variable):
    ...
    def formula(benunit, period, parameters):
        def active_at_date(date):
            return parameters(
                date
            ).hmrc.tax_credits.child_tax_credit.elements.family.active

        amount = parameters(
            period
        ).hmrc.tax_credits.child_tax_credit.elements.family.amount
        ...
        element_is_payable = benunit.max(
            person("is_child_for_ctc", period) * payable_at_birth
        )
